...
# ==========================================
GIGA_CONFIG = {
    "person_id_dev": "4c36eb04-0920-4449-9e07-ca4a68f80eef",
    "client_id": "fakeuser",
    "model": "GigaChat-2-Max",
    "token_url_dev": "https://hr-dev.sberbank.ru/auth/realms/PAOSberbank/protocol/openid-connect/token",
    "api_url_dev": "https://hr-dev.sberbank.ru/api-web/neurosearchbar/api/v1/gigachat/completion",
}
...
class BlastAIAssistant:
    ...
    def _setup_graph(self):
        ...
        def get_jira_status(issue_key: str) -> str:
            """Проверяет текущий статус задачи или релиза в Jira."""
        ...
        def create_business_requirements(issue_key: str, project_key: str) -> str:
            """Создает страницу Бизнес-требований (БТ) или Функционального решения (ФР) в Confluence."""
        ...
        def move_release_status(issue_key: str, target_status: str) -> str:
            """
            Переводит релиз/задачу Jira в целевой статус.
            Используй, когда пользователь просит: "переведи релиз в ...", "двинь статус", "закрой релиз".
            """
        ...
        def call_model(state: AgentState):
            ...
            sys_msg = SystemMessage(content=(
                "Ты DevOps AI Помощник, встроенный в приложение Blast.\\n"
                "У тебя есть арсенал инструментов-скриптов. Выбирай нужный:\\n"
                "1. Статус задачи/релиза -> 'get_jira_status' (нужен issue_key)\\n"
                ...
                "- НИКОГДА не используй массивы (списки) или блоки 'commands'. Каждый вызов инструмента должен быть самостоятельным словарём.\\n"
                "- Для смены статуса всегда используй инструмент 'move_release_status', а не текстовый ответ.\\n"
                "- Если пользователь просит полный релизный цикл (проверка LT + мастер + БТ + деплой), используй 'run_release_pipeline'.\\n"
                "- Если не хватает данных (например project_key для БТ) — спроси их текстом."
            ))
...
class ModernJiraApp(ctk.CTk):
    ...
    def create_operations_tab(self):
        ...
        input_frame = ctk.CTkFrame(self.operations_tab)
        ...
        self.release_entry = ctk.CTkEntry(input_frame, width=300, font=ctk.CTkFont(size=14), placeholder_text="HRPRELEASE-")
        self.release_entry.grid(row=0, column=1, padx=10, pady=10, sticky="w")
        ...
        self.version_entry = ctk.CTkEntry(input_frame, width=300, font=ctk.CTkFont(size=14), placeholder_text="Версия релиза")
        self.version_entry.grid(row=1, column=1, padx=10, pady=10, sticky="w")
        ...
        status_panel = ctk.CTkFrame(self.operations_tab)
        ...
        options_frame = ctk.CTkFrame(self.operations_tab)
        ...
        buttons_frame = ctk.CTkFrame(self.operations_tab)
        ...
        self.cancel_btn.grid(row=3, column=0, columnspan=3, padx=5, pady=5, sticky="ew")
        ...
        progress_frame = ctk.CTkFrame(self.operations_tab)
        progress_frame.pack(fill="x", padx=20, pady=10)

        self.progress_label = ctk.CTkLabel(progress_frame, text="", font=ctk.CTkFont(size=16, weight="bold"))
        ...
        results_frame = ctk.CTkFrame(self.operations_tab)
...
• Детальное логирование"""
    ...
    def refresh_release_status(self):
        ...
        if not issue:
            ...
            return
        status = issue.get("fields", {}).get("status", {}).get("name", "Unknown")
        self.release_status_value.configure(text=status, text_color="#2E7D32")
